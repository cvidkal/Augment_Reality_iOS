...
Feature_Match::Feature_Match(string featureType, int clusterCount)
:_featureType(featureType), _clusterCount(clusterCount)
{
...
void Feature_Match::matchNimages(Mat image, int n, vector<Matches> &matches, int &keyPointSize, vector<int> &matchPointSize, double &detectfps, double &matchfps)
{
	...
	tm.reset();
	tm.start();
    vtMatcher->match(descriptor, n, matchIdxs);
	tm.stop();
	...
	flann->train();
	vector<DMatch> matchLists;
	tm.reset();
	tm.start();
	flann->match(descriptor, matchLists);
	tm.stop();
